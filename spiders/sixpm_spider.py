...
class SixPMParseSpider(Spider):
    ...
    def product_skus(self, response):
        ...
        for index, (stock, size) in enumerate(zip(stock_data, size_data)):
            ...
            sku["sku_id"] = self.product_sku_id(sku["colour"], index)
    ...
    def product_sku_id(self, colour, index):
        return f"{colour.replace(' ', '')}_{index}"

    def product_colour(self, response):
        return response.css("#buyBoxForm span")[1].css("::text").get()
